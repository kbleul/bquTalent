...
const OurRange = () => {
    ...
    return (
        <article style={{
        ...
        }} className='py-10  mt-12 md:mt-36 block'>
            <h2 className="font-bold text-xl md:text-4xl text-center text-white w-full">Our Range of Specialized Outsourcing Services</h2>
            ...
            <section className="hidden md:grid grid-cols-3 gap-y-10 items-center justify-center mt-16 w-4/5 ml-[10%]">
                {PRO.map((item, index) => (<div key={item.name} className="bg-gray-100 text-center py-14 w-[70%] ml-[15%] rounded-2xl flex flex-col items-center justify-center">
                    ...
                    <h2 className="font-bold text-[#122638] text-sl">{item.name}</h2>
                    <p className=" px-4">{item.para}</p>
                </div>
                ))}
            </section>


            <section onTouchEnd={handleScroll} className=" md:hidden  mt-16 w-4/5 ml-[10%]">
                <div key={PRO[active].name} className="bg-gray-100 text-center py-14 w-full rounded-2xl flex flex-col items-center  justify-center">

                    <img className="  w-16 h-16" src={PRO[active].img} alt="" />

                    <h2 className="font-bold text-xl text-[#122638]">{PRO[active].name}</h2>
                    <p className="px-4  pt-1">{PRO[active].para}</p>

                    {/* <div className="details text-[#003366] ml-[13%]">
            ...
            <section onTouchEnd={handleScroll} className='py-6 flex items-center justify-center mt-10 gap-x-2 md:hidden '>
                {PRO.map((item, index) => (
                    <Dots key={index} active={active === index} bg={"white"} />
                ))}
            </section>
